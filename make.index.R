...
one.line<-"slides[SLIDE.NO] = new Array(2, \"Slide SLIDE.NO\", \"./slide-SLIDE.NO.svg\");"
...
function Navigate(slide_idx)
{
  ...
  if ( current_idx == slides.length) { 
    return;
    current_idx = 1;   //this is where we stop!!
  }
...
"  
make.index<-function(count){
  
  ALL.SLIDES<-sapply(1:count, function(x){gsub("SLIDE.NO",as.character(x),
                                               one.line,fixed=T)})
  ALL.SLIDES<-paste(ALL.SLIDES,collapse="\n")
  index.html<-gsub("ALL.SLIDES", ALL.SLIDES, slide.show.txt)
  index.html
}
